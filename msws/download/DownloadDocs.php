...
$rrefId = $_REQUEST['refId'];
$robjectId = $_REQUEST['objectId'];



if ($robjectId == 4) {
    $rconsultantId = $_REQUEST['consultantId'];
...
//
//        $conn->selectDatabase();
//        $Query = "SELECT TIMESTAMPDIFF(HOUR,AppliedDate, CURRENT_TIMESTAMP) AS difference FROM tblResourceDepotDetails WHERE Id=$rconsultantId";
...
//        }
//        if ($hoursDiff <= 48) {
             $rrefferenceId = $_REQUEST['refferenceId'];
            header('Location: ../events/webinar-after.php?eventId=' . $rrefId . '&objectId=' . $robjectId . '&refId=' . $rrefferenceId . '&consultantId=' .$rconsultantId);
     //   } else {
//            $rrefferenceId = $_REQUEST['refferenceId'];
//            header('Location: ../resource/get-resource.php?refId=' . $rrefferenceId . '&objectId=' . $robjectId . '&eventId=' . $rrefId);
        }
//    } catch (Exception $exc) {
//        $exc->getMessage();
...
//-----
  ...
  //  require '../config/DbController.php';
    ...
    try {
      ...
      //  $conn->selectDatabase();
        //$selectQuery = "SELECT DocName,DocLocation FROM tblWebDownloadsLines WHERE Id =$rrefId AND Download_Id = $robjectId";
        $selectQuery = "SELECT DocName,DocLocation FROM tblWebDownloadsLines WHERE Id =:rrefId AND Download_Id = :robjectId";
