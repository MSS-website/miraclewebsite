   ...
   <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.6.3/css/font-awesome.min.css">
    ...
    if (isset($_POST['btnSearchSubmit'])) {
    	...
    	try {
    		...
    		$searchQuery= 'SELECT PartnerName,OrganizationName,Designation,Email,PhoneNumber,ReasonTobePartner, DATE(CreatedDate) AS CreatedDate FROM tblDs17Partners WHERE 1=1 ';
    		...
    		if ($partnerName!= '') {
    			...
    			$searchQuery= $searchQuery. ' AND PartnerName LIKE :tempPartnerName';
...
<div class="row">
              ...
              <input type="text" class="form-control" name="partnerName" id="partnerName" value=""  placeholder="Partner Name">
                     
               </div> 
                <div class="col-sm-3">
                
                 <input class="form-control" type="text" name="organizationName" id="organizationName"  value="" placeholder="Organization Name">    
                 ...
                 <div class="col-sm-3">
                   ...
                   <div class="col-sm-3">
                     ...
                     <button type="button" onclick="exportPartners();" class="btn btn-success col-sm-12"> <i class="fa fa-download"></i>&nbsp;&nbsp;&nbsp;Export</button>
                 </div>
            </div>
</form>
...
<div class="row">
                              ...
                              <div class="gap" style="line-height: 20px; height: 20px;"></div>                                         
                                   <table class="table table-hover table-striped" id="reg_tab">
                                        ...
                                        try {
                                        	...
                                        	if($searchQuery==""){
                                        	$selectQuery = "SELECT PartnerName,OrganizationName,Designation,Email,PhoneNumber,ReasonTobePartner,Date(Createddate) as CreatedDate  FROM tblDs17Partners ORDER BY id ";
                                        	}else {
                                        		$selectQuery = $searchQuery;
                                        	}
                                        
                                        	$stmt = $conn->prepare($selectQuery);
                                        	if($partnerName!=""&&$searchQuery!=""){
                                        		$tempPartnerName= '%'.$partnerName.'%';
                                        		//$stmt->bindParam(':tempPartnerName', $partnerName);
                                        		$stmt->bindParam(':tempPartnerName', $tempPartnerName);
                                        	}
                                        	if($organizationName!=""&&$searchQuery!=""){
                                        		$tempOrg = "%" .$organizationName. "%";
                                        	...
                                        	while ($row = $stmt->fetch()) {
                                        		...
                                        		$ReasonTobePartner=$row["ReasonTobePartner"];
                                             ...
                                             <td  data-toggle="modal" data-target="#editModal"><?php echo $PartnerName; ?></td>
             ...
             <input type="hidden" id="frmPartnerName" name="frmPartnerName" value="<?=$partnerName;?>"/>
             <input type="hidden" id="frmOrganizationName" name="frmOrganizationName" value="<?=$organizationName;?>"/>
<!-- Vendor: Javascripts -->
...
$(document).ready(function () {
    $('#reg_tab').DataTable();
$("#partnerName").val($("#frmPartnerName").val());
$("#organizationName").val($("#frmOrganizationName").val());
    

});

function exportPartners() {
	window.location="export-partners.php";
}
